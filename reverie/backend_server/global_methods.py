...
def create_folder_if_not_there(curr_path):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <create_folder_if_not_there> {curr_path}")
...
def write_list_of_list_to_csv(curr_list_of_list, outfile):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <write_list_of_list_to_csv> {outfile}")
...
def write_list_to_csv_line(line_list, outfile):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <write_list_to_csv_line> {outfile}")
    create_folder_if_not_there(outfile)
...
def read_file_to_list(curr_file, header=False, strip_trail=True):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <read_file_to_list> {curr_file}")
    if not header:
        analysis_list = []
...
def read_file_to_set(curr_file, col=0):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <read_file_to_set> {curr_file}")
    analysis_set = set()
    with open(curr_file) as f_analysis_file:
        data_reader = csv.reader(f_analysis_file, delimiter=",")
        for count, row in enumerate(data_reader):
...
def get_row_len(curr_file):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <get_row_len> {curr_file}")
    try:
        analysis_set = set()
        with open(curr_file) as f_analysis_file:
            data_reader = csv.reader(f_analysis_file, delimiter=",")
...
def check_if_file_exists(curr_file):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <check_if_file_exists> {curr_file}")
    try:
        with open(curr_file) as f_analysis_file:
            pass
        return True
...
def find_filenames(path_to_dir, suffix=".csv"):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <find_filenames> {path_to_dir}")
...
def average(list_of_val):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <average> {list_of_val}")
...
def std(list_of_val):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <std> {list_of_val}")
...
def copyanything(src, dst):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <copyanything> {src} {dst}")
    try:
