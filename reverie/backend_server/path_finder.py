...
def path_finder_v2(a, start, end, collision_block_char, verbose=False):
    if debug:
        logger.info(f"[INFO]: <path_finder_v2_2> large arrays")
...
def path_finder(maze, start, end, collision_block_char, verbose=False):
    if debug:
        logger.info(f"[INFO]: <path_finder> large array")
    # EMERGENCY PATCH
...
def closest_coordinate(curr_coordinate, target_coordinates):
    if debug:
        logger.info(
            f"[INFO]: <closest_coordinate> {curr_coordinate} {target_coordinates}"
        )
    min_dist = None
...
def path_finder_2(maze, start, end, collision_block_char, verbose=False):
    if debug:
        logger.info(f"[INFO]: <path_finder_2_3> large arrays")
    # start => persona_a
    # end => persona_b
    start = list(start)
    end = list(end)
...
def path_finder_3(maze, start, end, collision_block_char, verbose=False):
    ...
    # end => persona_b
    if debug:
        logger.info(f"[INFO]: <path_finder_3> large array")
