...
class Maze:
    def __init__(self, maze_name):
        ...
        if debug:
            logger.info(f"[INFO]: <Maze> 1 {self.tiles}")
            logger.info(f"[INFO]: <Maze> 2 {self.address_tiles}")
    ...
    def turn_coordinate_to_tile(self, px_coordinate):
        ...
        """
        if debug:
            logger.info(f"[INFO]: <turn_coordinate_to_tile> {px_coordinate}")
    ...
    def get_tile_path(self, tile, level):
        ...
        """
        if debug:
            logger.info(f"[INFO]: <get_tile_path> {tile} {level}")
        x = tile[0]
        y = tile[1]
        tile = self.tiles[y][x]
    ...
    def get_nearby_tiles(self, tile, vision_r):
        ...
        """
        if debug:
            logger.info(f"[INFO]: <get_nearby_tiles> {tile} {vision_r}")
        left_end = 0
    ...
    def add_event_from_tile(self, curr_event, tile):
        ...
        """
        if debug:
            logger.info(f"[INFO]: <add_event_from_tile> {curr_event} {tile}")
    ...
    def remove_event_from_tile(self, curr_event, tile):
        ...
        """
        if debug:
            logger.info(f"[INFO]: <remove_event_from_tile> {curr_event} {tile}")
    ...
    def turn_event_from_tile_idle(self, curr_event, tile):
        if debug:
            logger.info(f"[INFO]: <turn_event_from_tile_idle> {curr_event} {tile}")
        curr_tile_ev_cp = self.tiles[tile[1]][tile[0]]["events"].copy()
        for event in curr_tile_ev_cp:
            if event == curr_event:
                self.tiles[tile[1]][tile[0]]["events"].remove(event)
    ...
    def remove_subject_events_from_tile(self, subject, tile):
        ...
        """
        if debug:
            logger.info(f"[INFO]: <remove_subject_events_from_tile> {subject} {tile}")
        curr_tile_ev_cp = self.tiles[tile[1]][tile[0]]["events"].copy()
        for event in curr_tile_ev_cp:
            if event[0] == subject:
                self.tiles[tile[1]][tile[0]]["events"].remove(event)
