...
def get_random_alphanumeric(i=6, j=6):
    ...
    """
    if debug:
        logger.info(f"[INFO] <get_random_alphanumeric> i: {i}, j: {j}")
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
    ...
    """
    if debug:
        logger.info(f"[INFO] <run_gpt_prompt_wake_up_hour> persona: {persona}")
...
def run_gpt_prompt_daily_plan(persona, wake_up_hour, test_input=None, verbose=False):
    ...
    if debug:
        logger.info(
            f"[INFO] <run_gpt_prompt_daily_plan> persona: {persona}, "
            f"wake_up_hour: {wake_up_hour}"
        )
...
def run_gpt_prompt_generate_hourly_schedule(
    ...
    if debug:
        logger.info(
            f"[INFO] <run_gpt_prompt_generate_hourly_schedule> persona: "
            f"{persona}, curr_hour_str: {curr_hour_str}, "
            f"p_f_ds_hourly_org: {p_f_ds_hourly_org}, hour_str: {hour_str}, "
            f"intermission2: {intermission2}"
        )
    ...
    def create_prompt_input(
        ...
        if debug:
            logger.info(
                f"[INFO] <create_prompt_input> persona: {persona}, "
                f"curr_hour_str: {curr_hour_str}, "
                f"p_f_ds_hourly_org: {p_f_ds_hourly_org}, "
                f"hour_str: {hour_str}, intermission2: {intermission2}, "
    ...
    if debug or verbose:
        logger.info(
            f"[INFO] <run_gpt_prompt_generate_hourly_schedule> output: " f"{output}"
        )
...
def run_gpt_prompt_action_sector(
    ...
    if debug:
        logger.info(
            f"[INFO] <run_gpt_prompt_action_sector> action_description: "
            f"{action_description}, persona: {persona}, maze: {maze}"
        )
