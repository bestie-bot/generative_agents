...
class Scratch:
    def __init__(self, f_saved):
        if debug:
            logger.info("[INFO]: [Scratch] Initializing Scratch.", f_saved)
        ...
        logger.info(f"[INFO]: [Scratch] Scratch initialized for {self.name}.")

        if check_if_file_exists(f_saved):
            if debug:
                logger.info("[INFO]: [Scratch] Loading Scratch.")
            ...
            logger.info(f"[INFO]: [Scratch] Scratch loaded for {self.name}.")
    ...
    def save(self, out_json):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Saving Scratch.")
        scratch = dict()
        ...
        try:
            log_message = "[INFO]: [Scratch] Adding new action\n"
            for key, value in scratch.items():
                log_message += f"{key}: {value}\n"
            logger.info(log_message)
        except Exception as e:
            logger.info(f"[ERROR]: [Scratch] Adding new action error \n{e}")
    ...
    def get_f_daily_schedule_index(self, advance=0):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Getting f_daily_schedule index.")
    ...
    def get_f_daily_schedule_hourly_org_index(self, advance=0):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Getting f_daily_schedule_hourly_org index.")
        # We first calculate teh number of minutes elapsed today.
        today_min_elapsed = 0
        today_min_elapsed += self.curr_time.hour * 60
        today_min_elapsed += self.curr_time.minute
    ...
    def get_str_iss(self):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Getting Identity Stable Set (ISS).")
        commonset = ""
    ...
    def get_str_name(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting name.")
        return self.name

    def get_str_firstname(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting first name.")
        return self.first_name

    def get_str_lastname(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting last name.")
        return self.last_name

    def get_str_age(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting age.")
        return str(self.age)

    def get_str_innate(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting innate.")
        return self.innate

    def get_str_learned(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting learned.")
        return self.learned

    def get_str_currently(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting currently.")
        return self.currently

    def get_str_lifestyle(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting lifestyle.")
        return self.lifestyle

    def get_str_daily_plan_req(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting daily plan requirement.")
        return self.daily_plan_req

    def get_str_curr_date_str(self):
        # if debug:
        #     logger.info("[INFO]: [Scratch] Getting current date string.")
        return self.curr_time.strftime("%A %B %d")

    def get_curr_event(self):
        if debug:
            logger.info("[INFO]: [Scratch] Getting current event.")
    ...
    def get_curr_event_and_desc(self):
        if debug:
            logger.info("[INFO]: [Scratch] Getting current event and description.")
        if not self.act_address:
    ...
    def get_curr_obj_event_and_desc(self):
        if debug:
            logger.info("[INFO]: [Scratch] Getting current object event and description.")
        if not self.act_address:
    ...
    def add_new_action(
        ...
        if debug:
            logger.info(
                "[INFO]: [Scratch] Adding new action\n"
                f"action_address: {action_address}\n"
                f"action_duration: {action_duration}\n"
                f"action_description: {action_description}\n"
                f"action_pronunciation: {action_pronunciatio}\n"
                f"action_event: {action_event}\n"
                f"chatting_with: {chatting_with}\n"
                f"chat: {chat}\n"
                f"chatting_with_buffer: {chatting_with_buffer}\n"
                f"chatting_end_time: {chatting_end_time}\n"
                f"act_obj_description: {act_obj_description}\n"
                f"act_obj_pronunciation: {act_obj_pronunciatio}\n"
                f"act_obj_event: {act_obj_event}\n"
                f"act_start_time: {act_start_time}"
            )
    ...
    def act_check_finished(self):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Checking if action is finished.")
        if not self.act_address:
            return True
    ...
    def act_summarize(self):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Summarizing action.")
        exp = dict()
    ...
    def act_summary_str(self):
        ...
        """
        if debug:
            logger.info("[INFO]: [Scratch] Getting action summary string.")
    ...
    def get_str_daily_schedule_summary(self):
        if debug:
            logger.info("[INFO]: [Scratch] Getting daily schedule summary string.")
        ret = ""
        curr_min_sum = 0
    ...
    def get_str_daily_schedule_hourly_org_summary(self):
        if debug:
            logger.info("[INFO]: [Scratch] Getting daily schedule hourly org summary string.")
        ret = ""
        curr_min_sum = 0
