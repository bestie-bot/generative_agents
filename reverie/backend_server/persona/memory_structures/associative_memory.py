...
class AssociativeMemory:
    def __init__(self, f_saved):
        if debug:
            logger.info("[INFO]: Initializing the associative memory.")
    ...
    def save(self, out_json):
        if debug:
            logger.info("[INFO]: Saving the associative memory.")
        r = dict()
    ...
    def add_event(
        ...
        if debug:
            logger.info(
                ...
                f"keywords: {keywords}\n"
                f"poignancy: {poignancy}\n"
                f"embedding_pair: {embedding_pair[0]}\n"
                f"filling: {filling}"
            )
    ...
    def add_thought(
        ...
        if debug:
            logger.info(
                f"[INFO]: New thought node created:\n"
                f"subject: {s}\n"
                f"predicate: {p}\n"
                f"object: {o}\n"
                f"description: {description}\n"
    ...
    def get_summarized_latest_events(self, retention):
        if debug:
            logger.info(f"[INFO]: Retrieving the latest {retention} events.")
        try:
            ret_set = set()
            ...
            logging.error(f"[ERROR]: [get_summarized_latest_events] {e}")
            pass

    def get_str_seq_events(self):
        if debug:
            logger.info("[INFO]: Retrieving the latest events.")
        try:
            ret_str = ""
            ...
            logging.error(f"[ERROR]: [get_str_seq_events] {e}")
            pass

    def get_str_seq_thoughts(self):
        if debug:
            logger.info("[INFO]: Retrieving the latest thoughts.")
        try:
            ret_str = ""
            ...
            logging.error(f"[ERROR]: [get_str_seq_thoughts] {e}")
            pass

    def get_str_seq_chats(self):
        if debug:
            logger.info("[INFO]: Retrieving the latest chats.")
        try:
            ret_str = ""
            ...
            logging.error(f"[ERROR]: [get_str_seq_chats] {e}")
            pass

    def retrieve_relevant_thoughts(self, s_content, p_content, o_content):
        if debug:
            logger.info("[INFO]: Retrieving relevant thoughts.")
        try:
            ...
            logging.error(f"[ERROR]: [retrieve_relevant_thoughts] {e}")
            pass

    def retrieve_relevant_events(self, s_content, p_content, o_content):
        if debug:
            logger.info("[INFO]: Retrieving relevant events.")
        try:
            contents = [s_content, p_content, o_content]
            ...
            logging.error(f"[ERROR]: [retrieve_relevant_events] {e}")
            pass

    def get_last_chat(self, target_persona_name):
        if debug:
            logger.info("[INFO]: Retrieving last chat.")
        try:
            ...
            logging.error(f"[ERROR]: [get_last_chat] {e}")
            pass
