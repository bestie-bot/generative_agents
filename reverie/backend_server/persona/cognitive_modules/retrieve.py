...
def retrieve(persona, perceived):
    ...
    if debug:
        logger.info(
            f"[INFO]: <retrieve> Retrieving perceived events for {persona.name}..."
        )
    retrieved = dict()
    for event in perceived:
        if debug:
            logger.info(
                f"[INFO]: <retrieve> Retrieving perceived event {event.description} for {persona.name}"
            )
    ...
    if debug:
        logger.info(
            f"[INFO]: <retrieve> Finished calculating perceived events for {persona.name}: {retrieved}"
        )
...
def cos_sim(a, b):
    ...
    if debug:
        logger.info(
            f"[INFO]: <cos_sim> Calculating cosine similarity between {a} and {b}"
        )
...
def normalize_dict_floats(d, target_min, target_max):
    ...
    if debug:
        logger.info(
            f"[INFO]: <normalize_dict_floats> Normalizing dictionary {d} between {target_min} and {target_max}"
        )
...
def top_highest_x_values(d, x):
    ...
    if debug:
        logger.info(
            f"[INFO]: <top_highest_x_values> Extracting top {x} values from dictionary {d}"
        )
...
def extract_recency(persona, nodes):
    ...
    if debug:
        logger.info(
            f"[INFO]: <extract_recency> Extracting recency scores from {nodes} for {persona.name}"
        )
    recency_vals = [
...
def extract_importance(persona, nodes):
    ...
    if debug:
        logger.info(
            f"[INFO]: <extract_importance> Extracting importance scores from {nodes} for {persona.name}"
        )
    importance_out = dict()
    for count, node in enumerate(nodes):
...
def extract_relevance(persona, nodes, focal_pt):
    ...
    if debug:
        logger.info(
            f"[INFO]: <extract_relevance> Extracting relevance scores from {nodes} for {persona.name}"
        )
...
def new_retrieve(persona, focal_points, n_count=30):
    ...
    if debug:
        logger.info(
            f"[INFO]: <new_retrieve> Retrieving nodes from {focal_points} for {persona.name}"
        )
    ...
    for focal_pt in focal_points:
        ...
        if debug:
            for key, val in master_out.items():
                logger.info(
                    f"[INFO]: <new_retrieve> {persona.name} {persona.a_mem.id_to_node[key].embedding_key} {val}"
                )
                logger.info(
                    f"[INFO]: <new_retrieve> {persona.scratch.recency_w * recency_out[key] * 1} {persona.scratch.relevance_w * relevance_out[key] * 1} {persona.scratch.importance_w * importance_out[key] * 1}"
                )
