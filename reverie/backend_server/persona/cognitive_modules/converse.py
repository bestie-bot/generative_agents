...
def generate_agent_chat_summarize_ideas(
    ...
    if debug:
        logger.info(
            f"[INFO]: [generate_agent_chat_summarize_ideas] Generating ideas for {init_persona} to {target_persona}."
        )
...
def generate_summarize_agent_relationship(init_persona, target_persona, retrieved):
    if debug:
        logger.info(
            f"[INFO]: [generate_summarize_agent_relationship] Summarizing relationship between {init_persona} and {target_persona}."
        )
    all_embedding_keys = list()
    for key, val in retrieved.items():
        for i in val:
...
def generate_agent_chat(
    ...
    if debug:
        logger.info(
            f"[INFO]: [generate_agent_chat] Generating chat between {init_persona} and {target_persona}."
        )
...
def agent_chat_v1(maze, init_persona, target_persona):
    ...
    if debug:
        logger.info(
            f"[INFO]: [agent_chat_v1] Generating chat between {init_persona} and {target_persona}."
        )
    curr_context = (
...
def generate_one_utterance(maze, init_persona, target_persona, retrieved, curr_chat):
    ...
    if debug:
        logger.info(
            f"[INFO]: [generate_one_utterance] Generating one utterance between {init_persona} and {target_persona}."
        )
    curr_context = (
        f"{init_persona.scratch.name} "
        + f"was {init_persona.scratch.act_description} "
...
def agent_chat_v2(maze, init_persona, target_persona):
    if debug:
        logger.info(
            f"[INFO]: [agent_chat_v2] Generating chat between {init_persona} and {target_persona}."
        )
    curr_chat = []
    print("July 23")
...
def generate_summarize_ideas(persona, nodes, question):
    if debug:
        logger.info(
            f"[INFO]: [generate_summarize_ideas] Generating ideas for {persona}."
        )
    statements = ""
    for n in nodes:
...
def generate_next_line(persona, interlocutor_desc, curr_convo, summarized_idea):
    if debug:
        logger.info(
            f"[INFO]: [generate_next_line] Generating next line for {persona}."
        )
...
def generate_inner_thought(persona, whisper):
    if debug:
        logger.info(
            f"[INFO]: [generate_inner_thought] Generating inner thought for {persona}."
        )
...
def generate_action_event_triple(act_desp, persona):
    ...
    """
    if debug:
        logger.info(f"INFO: [generate_action_event_triple] Generating triple.")
    return run_gpt_prompt_event_triple(act_desp, persona)[0]


def generate_poig_score(persona, event_type, description):
    if debug:
        logger.info(f"[INFO]: [generate_poig_score] Generating poignancy score.")
...
def open_convo_session(persona, convo_mode):
    if debug:
        logger.info(f"[INFO]: [open_convo_session] Opening conversation session.")
