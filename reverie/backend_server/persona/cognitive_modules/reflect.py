...
def generate_focal_points(persona, n=3):
    if debug:
        logger.info(
            f"[INFO]: <generate_focal_points> Generating focal points for {persona.scratch.name}"
        )
...
def generate_insights_and_evidence(persona, nodes, n=5):
    if debug:
        logger.info(
            f"[INFO]: <generate_insights_and_evidence> Generating insights and evidence for {persona.scratch.name}"
        )
    ...
    if debug:
        logger.info(f"[INFO]: <generate_insights_and_evidence> return {ret}")
    try:
...
def generate_action_event_triple(act_desp, persona):
    ...
    if debug:
        logger.info(
            f"[INFO]: <generate_action_event_triple> Generating action event triple for {persona.scratch.name}"
        )
...
def generate_poig_score(persona, event_type, description):
    if debug:
        logger.info(
            f"[INFO]: <generate_poig_score> Generating poignancy score for {persona.scratch.name}"
        )
...
def generate_planning_thought_on_convo(persona, all_utt):
    if debug:
        logger.info(
            f"[INFO]: <generate_planning_thought_on_convo> Generating planning thought on convo for {persona.scratch.name}"
        )
...
def generate_memo_on_convo(persona, all_utt):
    if debug:
        logger.info(
            f"[INFO]: <generate_memo_on_convo> Generating memo on convo for {persona.scratch.name}"
        )
...
def run_reflect(persona):
    ...
    Output:
      None
    """
    logger.info(f"[INFO]: <run_reflect> Running reflection for {persona.scratch.name}")
...
def reflection_trigger(persona):
    ...
    if debug:
        logger.info(
            f"[INFO]: <reflection_trigger> Checking reflection trigger for {persona.scratch.name} and persona.scratch.importance_trigger_curr {persona.scratch.importance_trigger_curr} with a max of {persona.scratch.importance_trigger_max}"
        )
...
def reset_reflection_counter(persona):
    ...
    if debug:
        logger.info(
            f"[INFO]: <reset_reflection_counter> Resetting reflection counter for {persona.scratch.name}"
        )
...
def reflect(persona):
    ...
    """
    if debug:
        logger.info(f"[INFO]: <reflect> Running reflection for {persona.scratch.name}")
    ...
    if persona.scratch.chatting_end_time:
        ...
        if debug:
            logger.info(
                f"[INFO]: <reflect> Generating planning and memo thoughts for {persona.scratch.name} at chatting end time {persona.scratch.chatting_end_time}"
            )
        if (
            ...
            # if debug:
            #     logger.info(f"[INFO]: <reflect> Init planning for {persona.scratch.name}'s planning: {planning_thought}")
            ...
            if debug:
                logger.info(
                    f"[INFO]: <reflect> Planning thought for {persona.scratch.name}'s planning: {planning_thought}"
                )
