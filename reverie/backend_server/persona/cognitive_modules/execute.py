...
def execute(persona, maze, personas, plan):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Executing {plan} for {persona.scratch.name}")
    ...
    if not persona.scratch.act_path_set:
        ...
        if "<persona>" in plan:
            ...
            if debug:
                logger.info(f"[INFO]: Executing persona-persona interaction")
            ...
            if debug:
                logger.info(f"[INFO]: Executing waiting action")
            ...
            if debug:
                logger.info(f"[INFO]: Executing random location action")
            ...
            if debug:
                logger.info(f"[INFO]: Executing default action")
