...
def generate_wake_up_hour(persona):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Generating wake up hour for {persona.scratch.name}")
...
def generate_first_daily_plan(persona, wake_up_hour):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Generating first daily plan for {persona.scratch.name}")
...
def generate_hourly_schedule(persona, wake_up_hour):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Generating hourly schedule for {persona.scratch.name}")
...
def generate_task_decomp(persona, task, duration):
    ...
    if debug:
        logger.info(
            f"[INFO]: Generating task decomposition for {persona.scratch.name}"
        )
...
def generate_action_sector(act_desp, persona, maze):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Generating action sector for {persona.scratch.name}")
...
def generate_action_arena(act_desp, persona, maze, act_world, act_sector):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Generating action arena for {persona.scratch.name}")
...
def generate_action_game_object(act_desp, act_address, persona, maze):
    ...
    if debug:
        logger.info(
            f"[INFO]: Generating action game object for {persona.scratch.name}"
        )
...
def generate_action_pronunciatio(act_desp, persona):
    ...
    if debug:
        logger.info(
            f"[INFO]: Generating action pronunciation for {persona.scratch.name}"
        )
    try:
...
def generate_action_event_triple(act_desp, persona):
    ...
    if debug:
        logger.info(
            f"[INFO]: Generating action event triple for {persona.scratch.name}"
        )
...
def generate_act_obj_desc(act_game_object, act_desp, persona):
    if debug:
        logger.info(
            f"[INFO]: Generating action object description for {persona.scratch.name}"
        )
...
def generate_act_obj_event_triple(act_game_object, act_obj_desc, persona):
    if debug:
        logger.info(
            f"[INFO]: Generating action object event triple for {persona.scratch.name}"
        )
...
def generate_convo(maze, init_persona, target_persona):
    if debug:
        logger.info(
            f"[INFO]: Generating conversation for {init_persona.scratch.name} and {target_persona.scratch.name}"
        )
...
def generate_convo_summary(persona, convo):
    if debug:
        logger.info(
            f"[INFO]: Generating conversation summary for {persona.scratch.name}"
        )
...
def generate_decide_to_talk(init_persona, target_persona, retrieved):
    if debug:
        logger.info(
            f"[INFO]: Generating decide to talk for {init_persona.scratch.name} and {target_persona.scratch.name}"
        )
...
def generate_decide_to_react(init_persona, target_persona, retrieved):
    if debug:
        logger.info(
            f"[INFO]: Generating decide to react for {init_persona.scratch.name} and {target_persona.scratch.name}"
        )
...
def generate_new_decomp_schedule(
    ...
    logger.info(
        f"[INFO]: Generating new decomposition schedule for {persona.scratch.name}"
    )
    ...
    for act, dur in p.scratch.f_daily_schedule:
        if (dur_sum >= start_hour * 60) and (dur_sum < end_hour * 60):
            ...
            if dur_sum <= today_min_pass:
                ...
                logger.info(
                    f"[INFO]: Daily schedule duration sum: {truncated_act_dur}"
                )
    ...
    if debug:
        logger.info(
            f"[INFO]: Decomposition completed for {persona.scratch.name}. Running GPT prompt."
        )
...
def revise_identity(persona):
    if debug:
        logger.info(f"[INFO]: Revising identity for {persona.scratch.name}")
    ...
    if debug:
        logger.info(
            f"[INFO]: Plan note created for {persona.scratch.name} as {plan_note}"
        )
    ...
    if debug:
        logger.info(
            f"[INFO]: Thought note created for {persona.scratch.name} as {thought_note}"
        )
    # print (thought_note)
    ...
    if debug:
        logger.info(
            f"[INFO]: Currently prompt created for {persona.scratch.name} as {currently_prompt}"
        )
    ...
    if debug:
        logger.info(
            f"[INFO]: New currently created for {persona.scratch.name} as {new_currently}"
        )
...
def _long_term_planning(persona, new_day):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Long term planning for {persona.scratch.name}")
    ...
    if debug:
        logger.info(
            f"[INFO]: <long_term_planning> Adding plan to memory for {persona.scratch.name}"
        )
...
def _determine_action(persona, maze):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Determining action for {persona.scratch.name}")
    ...
    def determine_decomp(act_desp, act_dura):
        ...
        if debug:
            logger.info(
                f"[INFO]: Determining decomposition for {persona.scratch.name}"
            )
    ...
    for i in persona.scratch.f_daily_schedule:
        logger.info(f"[INFO]: <for i in persona.scratch.f_daily_schedule> {i}")

    if debug:
        logger.info(
            f"[INFO]: For {persona.scratch.name} actions:: len {len(persona.scratch.f_daily_schedule)}"
        )
        logger.info(f"[INFO]: --------")
    ...
    if debug:
        logger.info(f"[INFO]: <x_emergency> {x_emergency}")

    if 1440 - x_emergency > 0:
        logger.info(f"[INFO]: <x_emergency__AAA> {x_emergency}")
...
def _choose_retrieved(persona, retrieved):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Choosing retrieved for {persona.scratch.name}")
...
def _should_react(persona, retrieved, personas):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Should react for {persona.scratch.name}")

    def lets_talk(init_persona, target_persona, retrieved):
        if debug:
            logger.info(
                f"[INFO]: Lets talk for {init_persona.scratch.name} and {target_persona.scratch.name}"
            )
    ...
    def lets_react(init_persona, target_persona, retrieved):
        if debug:
            logger.info(
                f"[INFO]: Lets react for {init_persona.scratch.name} and {target_persona.scratch.name}"
            )
        if (
            not target_persona.scratch.act_address
            or not target_persona.scratch.act_description
...
def _create_react(
    ...
    if debug:
        logger.info(f"[INFO]: Creating reaction for {persona.scratch.name}")
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas):
    if debug:
        logger.info(f"[INFO]: Chatting reaction for {persona.scratch.name}")
...
def _wait_react(persona, reaction_mode):
    if debug:
        logger.info(f"[INFO]: Waiting reaction for {persona.scratch.name}")
...
def plan(persona, maze, personas, new_day, retrieved):
    ...
    """
    if debug:
        logger.info(f"[INFO]: Planning for {persona.scratch.name}")
    ...
    if debug:
        logger.info(f"[INFO]: Planning for {persona.scratch.name} -- part 1 done")
    ...
    if debug:
        logger.info(f"[INFO]: Planning for {persona.scratch.name} -- part 2 done")
